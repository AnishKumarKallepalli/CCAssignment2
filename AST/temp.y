...
%type <ast> print_stmt <ast> dec_spec <ast> jump_stmt <ast> constant <ast> add_expr <ast> mul_expr <ast> rel_expr <ast>equ_expr <ast>log_or_expr <ast>log_and_expr <ast>arg_expr_list <ast>prim_expr <ast>postfix_expr <ast>unary_op <ast>unary_expr <ast>asg_expr expr  <ast>expr_stmt  <ast>id_list  <ast>param_list  <ast>init_list  <ast>init_dec  <ast>type_spec  <ast>declaration  <ast>iter_stmt  <ast>comp_stmt  <ast>lab_stat  <ast>sel_stmt  <ast>declarator  <ast>func_list  <ast>func_defn  <ast>stmt_list  <ast>stmt
    ...
    lab_stat
    	: CASE INTC CL stmt {$$ = create_node("case"); add_child($$, 2, create_node($2), $4);}
    	| DF CL stmt {$$ = create_node("default"); add_child($$, 1, $3);}
    	;
    ...
    print_stmt : PRINT OB STR CB SC {$$ = create_node("print"); add_child($$, 1, create_node($3)); fprintf(stderr, "%s\n", $3);}
               | PRINT OB STR CM id_list CB SC {$$ = create_node("print"); add_child($$, 2, create_node($3), $5); fprintf(stderr, "%s\n", $3);}
               ;
    %%
